...
class EditContractor extends React.Component {
  ...
  Refresh = () => {
    ...
    ContractorService.Get(this.props.match.params.id)
      .then((res) => {
        this.setState({
          organization: {
            ...res.data,
  ...
  GetDataList = () => {
    ...
    // ContractorService.GetAsSelectList().then((res) => {
  ...
  Search() {
    ...
    ContractorService.GetByInn(this.state.organization.inn)
      .then((res) => {
  ...
  handleChange(event, field, data) {
    ...
    if (!!event) {
      ...
      if (field == "bankId") {
        organization.bank = this.state.BankList.filter(
          (item) => item.value === organization.bankId
        )[0].text;
      }
      if (field == "okedId") {
  ...
  SaveData = () => {
    ...
    ContractorService.Update(this.state.organization)
      .then((res) => {
        this.setState({ SaveLoading: false });
        successToast(t2("SuccessSave", this.props.intl));

        setTimeout(() => {
          this.props.history.push("/document/contractor");
        }, 500);
      })
      .catch((error) => {
        errorToast(error.response.data);
            ...
            <Col sm={12} md={6} lg={4}>
              ...
              <InputGroup>
                <InputMask
                  ...
                  value={organization.innOrPinfl || ""}
                  onChange={(e) => this.handleChange(e, "innOrPinfl")}
                />
                <InputGroupAddon addonType="append">
                  <Button
                    disabled={!organization.innOrPinfl}
                    color="primary"
                    onClick={() => {
                      this.Search();
                    }}
            ...
            <Col sm={12} md={6} lg={4}>
              <FormGroup>
                ...
                <Select
                  ...
                  defaultValue={{
                    text: organization.bank || t2("Choose", intl),
                  }}
                  value={{
                    text: organization.bank || t2("Choose", intl),
                  }}
            ...
            <Col sm={12} md={6} lg={4}>
              <FormGroup>
                ...
                <InputMask
                  ...
                  onChange={(e) => this.handleChange(e, "contact", e)}
                />
                {/* <Input
                  type="text"
          ...
          <Row>
            <Col className="text-right">
              <Button
                className="mr-1"
                color="danger"
                onClick={() => history.push("/document/contractor")}
              >
                {" "}
                {t1("back")}{" "}
              </Button>
...
export default injectIntl(EditContractor);
