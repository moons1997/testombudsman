...
class EditBank extends React.Component {
  ...
  Refresh = () => {
    ...
    CountryService.Get(this.props.match.params.id)
      .then((res) => {
        this.setState({
          country: {
            ...res.data,
    ...
    if (!!event) {
      ...
      if (field == "stateId") {
        country.state = this.state.StateList.filter(
          (item) => item.value === country.stateId
        )[0].text;
      }
  ...
  SaveData = () => {
    ...
    CountryService.Update(this.state.country)
      .then((res) => {
        this.setState({ SaveLoading: false });
        successToast(t2("SuccessSave", this.props.intl));

        setTimeout(() => {
          this.props.history.push("/info/country");
        }, 500);
      })
      .catch((error) => {
        errorToast(error.response.data);
      ...
      <Overlay show={loading}>
        ...
        <Card className="p-2">
          ...
          <Row>
            ...
            <Col sm={12} md={6} lg={4}>
              <FormGroup>
                <h5 className="text-bold-600"> {t1("textCode")} </h5>
                <Input
                  type="text"
                  value={country.textCode || ""}
                  onChange={(e) => this.handleChange(e, "textCode")}
                  id="textCode"
                  placeholder={t2("textCode", intl)}
                />
              </FormGroup>
            </Col>
            {/* <Col sm={12} md={6} lg={4}>
          ...
          <Row>
            <Col className="text-right">
              <Button
                className="mr-1"
                color="danger"
                onClick={() => history.push("/info/country")}
              >
                {" "}
                {t1("back")}{" "}
              </Button>
