...
class ExcelReport3 extends React.Component {
  constructor(props) {
    super(props);
  ...
  GetExcel() {
    ...
    ExcelReportService.Report3AsExcel({
  ...
  forceFileDownload(response, name, attachfilename) {
    ...
    link.href = url;
    link.setAttribute("download", t2("ExcelReport3", intl) + "." + "xlsx");
        ...
        <Card className="p-2">
          <Row>
            <Col className="text-center">
              <h1 className="pageTextView"> {t1("ExcelReport3")}</h1>
            </Col>
          </Row>
          <Row className="mb-2">
            <Col md="2">
          ...
          {!!data?.regionItems ? (
            <Table responsive className="excel-table">
              ...
              >
                ...
                >
                  <td rowSpan={3}>№</td>
                  <td rowSpan={3}>{t1("place")}</td>
                  <td colSpan={5}>{t1("NotifiedExcel")}</td>
                      ...
                      true
                        ? item?.districtItems.map((district, id) => (
                            <tr style={{ textAlign: "center" }}>
                              ...
                              <td>{district.notified.all}</td>
                              <td>{district.notified.executed}</td>
                              <td>{district.notified.includingOn[0]?.count}</td>
                              <td>{district.notified.includingOn[1]?.count}</td>
                              <td>{district.notified.includingOn[2]?.count}</td>
                              <td>{district.notified.includingOn[3]?.count}</td>
                              <td>{district.notified.violationsDetected}</td>
                              <td>{district.allAgreeType.all}</td>
                              <td>
                                {district.allAgreeType.includingOn[0]?.count}
                              </td>
                              <td>
                                {district.allAgreeType.includingOn[1]?.count}
                              </td>
                              <td>{district.rejected}</td>

                              <td>{district.agreed.all}</td>
                              <td>{district.agreed.executed}</td>
                              <td>{district.agreed.includingOn[0]?.count}</td>
                              <td>{district.agreed.includingOn[1]?.count}</td>
                              <td>{district.agreed.violationsDetected}</td>
                              <td>{district.allExecuted}</td>
                              <td>{district.allViolationsDetected}</td>
                            </tr>
                          ))
                        : null
                    }
...
export default injectIntl(ExcelReport3);
