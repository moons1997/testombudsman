...
class EditBank extends React.Component {
  ...
  Refresh = () => {
    ...
    NationalityService.Get(this.props.match.params.id)
      .then((res) => {
        this.setState({
          nationality: {
            ...res.data,
  ...
  SaveData = () => {
    ...
    NationalityService.Update(this.state.nationality)
      .then((res) => {
        this.setState({ SaveLoading: false });
        successToast(t2("SuccessSave", this.props.intl));

        setTimeout(() => {
          this.props.history.push("/info/nationality");
        }, 500);
      })
      .catch((error) => {
        errorToast(error.response.data);
        ...
        <Card className="p-2">
          <Row>
            <Col className="text-center mb-1">
              <h1 className="pageTextView"> {t1("Nationality")} </h1>
            </Col>
          </Row>
          <Row>
            {/* <Col sm={12} md={6} lg={4}>
          ...
          <Row>
            <Col className="text-right">
              <Button
                className="mr-1"
                color="danger"
                onClick={() => history.push("/info/nationality")}
              >
                {" "}
                {t1("back")}{" "}
              </Button>
