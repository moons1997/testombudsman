...
class CheckingQuiz extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      fields: [
        ...
        { key: "question", label: t2("Question", props.intl), sort: true },
        {
  ...
  GetExcel() {
    ...
    CheckingQuizService.SaveAsExecel(this.state.filter)
      .then((res) => {
  ...
  forceFileDownload(response, name, attachfilename) {
    ...
    link.href = url;
    // if (format.length > 0) {
    link.setAttribute("download", t2("CheckingQuiz", intl) + "." + "xlsx");
    // }
        ...
        <Card className="p-2">
          ...
          <Row className="mt-2">
            ...
            <Col sm={12} md={12} lg={9} className="text-right">
              ...
              {can("CheckingQuizCreate") || can("AllCheckingQuizCreate") ? (
                <Button
                  color="primary"
                  onClick={() => history.push(`/info/editcheckingquiz/0`)}
                >
                  {" "}
          ...
          <Row className="mt-2">
            <Col>
              <WebaseTable
                fields={fields}
                filter={filter}
                api={CheckingQuizService}
                childRef={(ref) => (this.child = ref)}
                actions={() => {
                  return {
                    edit: {
                      isView:
                        can("CheckingQuizEdit") || can("AllCheckingQuizEdit"),
                      router: "/info/editcheckingquiz",
                    },
                    delete: {
                      isView:
                        can("CheckingQuizDelete") ||
                        can("AllCheckingQuizDelete"),
                    },
                  };
                }}
              />
...
export default injectIntl(CheckingQuiz);
