...
import styleee from "./styleee.css";
  ...
  displayNone = (item) => {
    if (!!item.children && item.children.length > 0) {
      ...
      for (var i = 0; i < item.children.length; i++) {
        ...
        if (typeof el.visible == "object") {
          if (el.visible.length > 0) {
            el.visible.forEach((element) => {
              s.push(can(element));
            });
          }
        }
        if (typeof el.visible == "string") {
          s.push(can(el.visible));
        }
      }
      if (s.filter((t) => t == true).length > 0) {
        return false;
      } else {
        return true;
      }
      ...
      if (typeof el.visible == "string") {
        w.push(can(el.visible));
      }
      if (w.filter((t) => t == true).length > 0) {
        return false;
      } else {
        return true;
      }
  ...
  render() {
          ...
          >
            ...
            >
              <div className="menu-text">
                ...
                <span style={{ width: "80%" }} className="menu-item menu-title">
    ...
    return (
      <React.Fragment>
        ...
        >
          <Icon.Home className="mr-1" size={18} /> {t1("Main")}
        </Button>
        {menuItems}
        {/* <a href="http://my.govcontrol.uz/" target="_blank">
          <Button
            style={{ width: "100%", height: "50px" }}
            className="mr-1 text-left"
            color="primary"
          >
            <Icon.Globe className="mr-1" size={18} /> {t1("myGovcontrolUz")}
          </Button>
        </a> */}
        <a href="https://t.me/tekshiruvinfo_bot" target="_blank">
          <Button
            style={{ width: "100%", height: "50px" }}
            className="mr-2 text-left"
            color="primary"
          >
            <Icon.Send className="mr-1" size={18} /> {t1("Telegram_Bot")}
          </Button>
        </a>
      </React.Fragment>
    );
